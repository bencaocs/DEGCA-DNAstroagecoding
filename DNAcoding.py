from __future__ import division#py2中的精确除
from __future__ import print_function#py2中的py3的print
...
# Define model evaluation function定义模型评价函数
...
time_limit = 12000 # time limit for searching
        ...
        if reduced_nn != 0:
            ...
            for out_id in range(noout):
                ...
                if np.sum(remain_vec_tmp) == 0:
                    ...
                    while round(time.time()-start_time)%5==0:
                        best_IS_num_List.append(best_IS_num)
